...
dload_and_install() {
  ...
  curl -sLo revanced-builder.zip https://github.com/Rikysonic/revanced-builder/archive/refs/heads/main.zip
  log "Unzipping..."
...
run_self_update() {
  ...
  ! curl -sLo "$SCR_NAME_EXEC_FP".tmp https://raw.githubusercontent.com/Rikysonic/revanced-builder/main/android-interface.sh && {
    log "Failed: Error while trying to download new version!"
    error "File requested: https://raw.githubusercontent.com/Rikysonic/revanced-builder/main/android-interface.sh" n
  } || log "Done."
